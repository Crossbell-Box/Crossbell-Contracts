...
contract Web3EntryStorage {
    // profileId => Profile
    mapping(uint256 => DataTypes.Profile)
        internal _profileById;
    // handleHash => profileId
    mapping(bytes32 => uint256)
        internal _profileIdByHandleHash;
    // address => profileId
    mapping(address => uint256)
        internal _primaryProfileByAddress;
    ...
    mapping(address => address)
        internal _linkModules4Address;

    // tokenAddress => tokenId => linkModule4ERC721
    mapping(address => mapping(uint256 => address))
        internal _linkModules4ERC721;

    // tokenId => linkModule4Linklist
    mapping(uint256 => address)
        internal _linkModules4Linklist;
}
